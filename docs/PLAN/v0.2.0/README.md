...
v0.2.0 是 Rowan 从单包 `src/` 走向模块化 runtime 的第一步。

它不是完整 v1.0.0 monorepo，也不是一次性完成所有包能力。v0.2.0 的目标是完成拆包条件，并开始把已经有清晰边界的模块迁出：
...
`packages/eval` 和 `packages/workflow` 在 v0.2.0 只保留接口方向和边界约束，不做完整实现。
