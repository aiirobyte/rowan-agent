...
> 状态：已实现
> 技术栈：TypeScript + Bun
> 基线：v0.1 Real Model Runtime
> 任务表：`docs/PLAN/v0.2/TASKS.md`

## 1. v0.2 目标

v0.2 的核心目标是完成 monorepo 拆包条件，并开始按照 `docs/PLAN/ARCHITECTURE.md` 的模块定义拆解。

v0.2 不是完整 v1.0。它只做最小、可回归、低风险的拆包：
...
`packages/eval` 和 `packages/workflow` 在 v0.2 只定义未来边界，不进入主执行链路。
...
| 条件 | 当前状态 | v0.2 处理方式 |
|---|---|---|
| v0 API 稳定 | 基本稳定，但需要显式冻结 public API | 建立 `agent` public exports，迁移 import 到 package 入口 |
| 至少一个真实模型 adapter 完成 | OpenAI-compatible 已实现 | 迁入 `packages/adapters` |
| workspace ACI 开始引入多工具 | 尚未开始 | 新增 `packages/aci` 最小工具集 |
| trace 不再只是 writer，需要 reader/replay | writer 已有，reader/replay 未有 | v0.2 做 reader + inspect，replay/fork 后置 |
...
v0.2 需要显式冻结一层 public API，并以 package 入口作为唯一稳定入口：
...
v0.2 不保留根 `src/index.ts` 兼容导出。原因是当前项目还没有对外发布 npm package，也没有需要承诺的旧 import surface；保留旧入口会让后续边界变模糊。
...
v0.2 的 ACI 目标是提供工具协议，不是做完整 coding agent。
...
- `docs/PLAN/v0.2/PLAN.md` 完成。
- `docs/PLAN/v0.2/TASKS.md` 完成。
...
- trace schema 与 v0.1 兼容。

## 10. 风险和约束

| 风险 | 影响 | 缓解 |
|---|---|---|
| 过早拆太细 | package 边界反复变化 | v0.2 只拆已有压力的 agent/adapters/trace/aci/cli |
| 循环依赖 | build 和测试不稳定 | 明确 dependency direction，并加检查 |
| CLI 兼容破坏 | 用户现有命令失效 | 根 scripts 保持不变 |
| Trace schema 漂移 | 历史 run 不可读 | v0.2 不改事件语义 |
