...
| V032-101 | M1 | Rename Session `userInput` to immutable `input` | session | P0 | V032-001 | done | persisted session and session snapshots use `input`; append turn does not mutate it |
...
| V032-203 | M2 | Rewire `runSubSession()` / `startSubSession()` to thread implementation | agent | P0 | V032-202 | done | old tests and public API work through the new path |
| V032-301 | M3 | Extend route schema to direct/task/thread | agent | P0 | V032-103 | done | parser accepts new `route` field and old `needsTask` field |
...
| V032-402 | M4 | Update OpenAI-compatible normalizers | adapters | P0 | V032-401 | done | adapter normalizes new route schema and legacy route schema |
...
| V032-502 | M5 | Update trace inspector for thread events | trace | P1 | V032-501 | done | inspector lists parent/child relationships from new and old events |
...
- [x] compatibility sub-session tests
