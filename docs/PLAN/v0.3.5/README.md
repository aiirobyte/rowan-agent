...
6. `trace list` / `trace show` inspect 子命令移除，本版本不改名为 `log list` / `log show`。
7. 不引入 replay / fork / log inspect；后续需要回放时从 `AgentStore.steps` 和 source events 设计。
...
- CLI 不再暴露 `--trace`、`trace list`、`trace show`。
