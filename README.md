...
`bun run rowan [options] [prompt]` is the single CLI entrypoint. When a prompt is provided, Rowan runs it first and then continues reading interactive turns from stdin/TTY. Controls are `:session`, `:exit`, and `:quit`.
